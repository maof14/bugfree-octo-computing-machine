Attribute VB_Name = "CreateQTCMSalesOrder"
Option Explicit
Dim sBar
...
Sub CreateQTCMSalesOrderScript(ByRef PONumber As String, ByRef Curr As String, ByRef WBSElement As String, ByRef Descr As String, ByRef RAKey As String, ByRef trx As String, ByRef SAPConnection As Variant, ByRef CMail As Variant)
crtdByStp = False
...
' Man skulle kunna ha som setting på förstasidan att "jag har VC / jag har inte VC"
    ...
    SAPConnection.session.findById("wnd[0]/usr/subSUBSCREEN_HEADER:SAPMV45A:4021/txtVBKD-BSTKD").Text = PONumber ' PO-number (variable)
        Do
...
'    SAPConnection.session.findById("wnd[0]").sendVKey 0
    ...
    SAPConnection.session.findById("wnd[0]/usr/tabsTAXI_TABSTRIP_HEAD/tabpT\12").Select
