Attribute VB_Name = "RG_POC_Milestone_Creation"
Sub RG_POC_Milestone_CreationScript(ByRef WBS As String, ByRef POC As String, ByRef ActDate As String, ByRef Supplink As String, ByRef trx As String, ByRef SAPConnection As Variant, ByRef CMail As Variant)
    Dim sBar
    Dim oName
    Dim MileDesc
    Dim RequestorX
    ...
    RequestorX = UCase(Right(Range("B4").value, 7))
    MileDesc = POC & "% POC requested by " & RequestorX & " - " & Format(Now(), "YYMM") & "A"
    ...
    End If
        If InStr(sBar.Text, "locked") > 0 Then
            ActiveCell.value = 0
            Cells(ActiveCell.Row, 7).value = "WBS BLOCKED by another user, try again later."
            Exit Sub
        End If
            ...
            If InStr(oName.Text, "CLSD") > 0 Then
                        ActiveCell.value = 1
            Cells(ActiveCell.Row, 7).value = "WBS is closed, cannot maintain."
            Exit Sub
            End If
    ...
    ActiveCell.Offset(0, 9).value = sBar.Text
    ActiveCell.value = 1
    Exit Sub
